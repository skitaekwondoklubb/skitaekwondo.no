...
export function AgendaRow(props: AgendaRowProps) {
    const single = props.row.happenings.length === 1;
    const rowIsShown = props.row.happenings
    .filter(x => props.showTypes === ShowTypes.All || (x.type === ShowTypes.All || props.showTypes === x.type))
    .filter(x => (props.showGrade === null || x.who.length === 0 || (x.who.find(y => y.color === props.showGrade?.color) != null))).length > 0;

    if(rowIsShown) {
        return (
            <div className={styles.agendaRow}>
                <label className={styles.bold}>{props.row.when}</label> 
                <div className={single ? styles.agendaCommonBox : styles.agendaMultiBox}>
                    {
                        props.row.happenings
                        .filter(x => props.showTypes === ShowTypes.All || (x.type === ShowTypes.All || props.showTypes === x.type))
...
function MultiBox(props: AgendaBoxProps) {
    return (
        <div className={`${styles.agendaBox} ${props.agenda.class} ${props.single ? styles.singleBox : styles.normalBox}`}>
            ...
            { props.agenda.who.length === 0 && "Alle" }
            <p>{ props.agenda.who.length > 0 && findGradeSpan(props.agenda.who) }</p>
